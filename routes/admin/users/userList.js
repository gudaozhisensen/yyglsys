...
router.get('/addUser', async (req, res) => {
  ...
  res.render('admin/users/addUser.ejs', { roles });
});
...
router.post('/delUser', (req, res) => {
  ...
  delId.forEach(item => {
    ...
    let sqlStr1 = 'delete from doctor where userid = ?';
    let arr = [item];
    queryFn(sqlStr, arr);
    queryFn(sqlStr1, arr);
  });
...
router.post('/addUser', async (req, res) => {
  ...
  let sqlStr =
    'insert into user (username,name,phone_number,email,password,roleid) values (?,?,?,?,?,?)';
  await queryFn(sqlStr, [
    username,
    name,
    phone_number,
    email,
  ...
  //判断添加的是什么用户,向相应的表添加人
  let sqlStr1 = `select id from user where username = '${username}'`;
  let userid = await queryFn(sqlStr1);
  let arr = [username, name, userid[0].id];
  await addFn(roleid, arr);
});
...
//根据角色添加到相应的表--------------------------------------------
async function addFn(roleid, arr) {
  if (roleid == 43) {
    let sqlStr = `insert into doctor (username,doctorname,userid) values (?,?,?)`;
