...
router.get('/manage', async (req, res) => {
  ...
  if (searchItem != undefined && cid == undefined) {
    let str =
      'SELECT d.ID,d.drugname,d.company,d.brief,d.price,d.items,d.entrytime,c.catagory FROM drugs AS d INNER JOIN drugs_catagory AS c ON d.catagory = c.ID where d.drugname like ? or d.company like ? limit ? ,10';
    ...
    let allPage = await mySql(
      'select count(*) as allpage from drugs as d where d.drugname like ? or d.company like ?',
    ...
    let str =
      'SELECT d.ID,d.drugname,d.company,d.brief,d.price,d.items,d.entrytime,c.catagory FROM drugs AS d INNER JOIN drugs_catagory AS c ON d.catagory = c.ID where (d.drugname like ? or d.company like ?) and d.catagory = ? limit ? ,10';
    let result = await mySql(str, [
      '%' + searchItem + '%',
      '%' + searchItem + '%',
      cid,
    ...
    let allPage = await mySql(
      'select count(*) as allpage from drugs as d where (d.drugname like ? or d.company like ?) and d.catagory = ?',
      ['%' + searchItem + '%', '%' + searchItem + '%', cid]
    );
    allPage = parseInt(allPage[0].allpage);
    let allnum = allPage;
    allPage = Math.ceil(allPage / 10);
    ...
    let str =
      'SELECT d.ID,d.drugname,d.company,d.brief,d.price,d.items,d.entrytime,c.catagory FROM drugs AS d INNER JOIN drugs_catagory AS c ON d.catagory = c.ID where d.catagory = ? limit ? ,10';
    let result = await mySql(str, [cid, (page - 1) * 10]);
    let allPage = await mySql(
      'select count(*) as allpage from drugs where catagory = ?',
      [cid]
    );
    allPage = parseInt(allPage[0].allpage);
    let allnum = allPage;
    ...
    let str =
      'SELECT d.ID,d.drugname,d.company,d.brief,d.price,d.items,d.entrytime,c.catagory FROM drugs AS d INNER JOIN drugs_catagory AS c ON d.catagory = c.ID limit ? ,10';
...
router.post('/searchDrugs', async (req, res) => {
  ...
  let allPage = await mySql(
    'select count(*) as allpage from drugs where drugname like ? or company like ?',
    ['%' + searchItem + '%', '%' + searchItem + '%']
  );
  allPage = parseInt(allPage[0].allpage);
  let allnum = allPage;
...
router.get('/adddrugs', async (req, res) => {
  ...
  if (ID == undefined) {
    ...
    let sqlStr =
      'SELECT c.ID,d.drugname,d.company,d.brief,d.price,d.items,d.entrytime,c.catagory FROM drugs AS d INNER JOIN drugs_catagory AS c ON d.catagory = c.ID where d.ID = ?';
...
router.post('/adddrugs', async (req, res) => {
  ...
  let roleid = await mySql('select * from drugs where username = ?', [
    username
  ]);
  ...
  if (Array.from(result).length == 0) {
    // 添加药品
    let insertSql =
      'insert into drugs (drugname,catagory,company,brief,price,items,author,roleid,entrytime) values(?,?,?,?,?,?,?)';
    ...
    // 修改药品
    let updateSql =
      'update drugs set catagory = ?,company = ?,brief = ?,price =?,items = ?,entrytime = ?,author = ?,roleid = ? where drugname = ?';
    await mySql(updateSql, [
      obj.catagory,
      obj.company,
      obj.brief,
      obj.price,
...
router.get('/catagory', async (req, res) => {
  ...
  if (searchItem != null) {
    ...
    let allPage = await mySql(
      'select count(*) as allpage from drugs_catagory where catagory = ?',
      [searchItem]
    );
...
router.post('/addcatagory', async (req, res) => {
  ...
  let sqlStr =
    'insert into drugs_catagory (catagory,confirmTime,author,roleid) values (?,?,?,?)';
